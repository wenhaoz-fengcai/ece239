...
class KNN(object):
  ...
  def compute_L2_distances_vectorized(self, X):
    ...
    dists = np.sqrt(test_sum.reshape(-1,1) +train_sum - 2*test_train)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in np.arange(num_test):
      ...
      # ================================================================ #
      index = sorted(range(dists.shape[1]),key = lambda j : dists[i,:][j])[:k]
  
      closest_y = [self.y_train[i] for i in index ]
