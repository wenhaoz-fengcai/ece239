...
class loader(object):
    ...
    def getAllDataSubject(self,num_test,num_validation):
        ...
        for i in range(9):
            ...
            dataset = TensorDataset(data_tensor, target_tensor)
            self.test_loaders.append(torch.utils.data.DataLoader(dataset, batch_size=50,
                                                       shuffle=True, sampler=None,
                                                       batch_sampler=None,
                                                       num_workers=0,
                                                       pin_memory=False, drop_last=False,
    ...
    def getLoader(self,X,y,num_test,num_validation):
        ...
        data_tensor = torch.Tensor(X_train.reshape(num_training, 22, 1000))
        target_tensor = torch.Tensor(y_train)
        ...
        data_tensor = torch.Tensor(X_test.reshape(num_test, 22, 1000))
        target_tensor = torch.Tensor(y_test)
        ...
        # validation
        data_tensor = torch.Tensor(X_val.reshape(num_validation, 22, 1000))
        target_tensor = torch.Tensor(y_val)
